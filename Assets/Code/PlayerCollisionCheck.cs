...
public class PlayerCollisionCheck : MonoBehaviour
{
    ...
    public LayerMask collisionLayer; // Để xác định các đối tượng mà Player sẽ va chạm với
    ...
    {
        // Dùng OverlapSphere để kiểm tra các đối tượng trong phạm vi xung quanh Player
        ...
        if (hitColliders.Length > 0)
        {
            foreach (var hitCollider in hitColliders)
            {
                ...
                {
                    Debug.Log("Player đã va chạm với Cube (mặt đất)");

                    // Bắt đầu hiệu ứng mờ dần
                    StartCoroutine(FadeAndDestroy(hitCollider.gameObject));
                }
            }
        }
    }
